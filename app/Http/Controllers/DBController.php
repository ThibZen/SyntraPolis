...
//    }
    ...
    {
        $result = DB::select("SELECT * FROM {$table} WHERE {$column} LIKE '$value'", [1]);
    ...
    {   $actorfirst  = $req->input ('actor_first');
        ...
        if ($this->record_exists_names('actor', $actorfull) == false) {
            ...
            if(DB::insert('$sqlActor')){
                ...
                $actor_id = sqlite_last_insert_rowid();
            } else {
                echo "ERROR: Could not able to execute $sqlActor. " . sqlite_last_error();
            }
        } else {
            //RECORD EXISTS
            $result = (DB::insert("SELECT ActorID FROM actor WHERE CONCAT(FirstName,' ',LastName) LIKE '%$actorfull%'"))->fetch_all()[0];
            $actor_id = $result[0];
        }
        return $actor_id;
    }
    ...
    function directorInsert(Request $req)
    {
        ...
        if ($this->record_exists_names('director', $directorfull) == false) {
            ...
            if (DB::insert('$sqlDirector')){
                ...
                $director_id = sqlite_last_insert_rowid();
            } else {
                echo "ERROR: Could not able to execute $sqlDirector. " . sqlite_last_error();
            }
        } else {
            //RECORD EXISTS
            $result = (DB::insert("SELECT DirectorID FROM director WHERE CONCAT(FirstName,' ',LastName) LIKE '%$directorfull%'"))->fetch_all()[0];
    ...
    function insert(Request $req){
        ...
        if ($this->record_exists_movie('movie','Title',$movie_title) == true) {
            ...
            if(DB::insert('$sqlMovie')){
                ...
                $movie_id = sqlite_last_insert_rowid();
                $actor_id = $this->actorInsert();
                $director_id = $this->directorInsert();
